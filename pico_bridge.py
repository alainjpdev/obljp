...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "--find-ports":
        ...
        elif command == "--execute-code":
            # Leer código desde stdin
            code = sys.stdin.read()
            bridge = PicoBridge()
            
            # Buscar puerto Pico automáticamente
            ports = bridge.find_ports()
            if not ports:
                print("PICO_ERROR: No se encontraron puertos Pico")
                sys.exit(1)
                
            # Conectar al primer puerto encontrado
            if bridge.connect(ports[0].device):
                # Ejecutar código
                bridge.execute_code(code)
            else:
                print("PICO_ERROR: No se pudo conectar al Pico")
                sys.exit(1)
            ...
            print("  --execute-code   : Ejecutar código desde stdin")
